...
class User extends \common\components\ActiveRecord implements IdentityInterface
{
    ...
    public static function table_chema( $migration )
    {
        return [
            ...
            'auth_key' => $migration->string(32)->notNull(),
            'password_hash' => $migration->string()->notNull(),
            'password_reset_token' => $migration->string()->unique(),
            'email' => $migration->string()->notNull()->unique(),

            'is_root' => $migration->boolean()->notNull(),
